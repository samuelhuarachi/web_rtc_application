...
var socket = null;
var analistVideo = document.getElementById("analist_video");
...
function connectSocket() {
  ...
  socket.on("connect", function () {
    ...
    console.log("analist id" + analistID); // socket.emit(
    //     "registerAnalist",
    //     JSON.stringify({
    //         token
    //     })
    ...
    socket.emit("register_analist", JSON.stringify({
      token: token
    }));
  });
}
...
socket.on("generateAnalistOffer", function (clientId) {
  ...
  if (pc) {//disconnectPeerByClientSocketID(clientId)
  }
